...
const mditCases = parseFixture(
  read("fixtures/ruby-mdit.md"),
  read("expected/ruby-mdit.html")
);
...
describe("ruby/mdit", () => {
  it.each(cmCases)("ruby (markdown-it): $name", ({ input, expected }) => {
    expect(normalizeHtml(md.render(input))).toBe(expected);
  });

  it.each(gfmCases)("ruby (markdown-it) gfm: $name", ({ input, expected }) => {
    expect(normalizeHtml(md.render(input))).toBe(expected);
  });

  it.each(mditCases)(
    "ruby (markdown-it) strikethrough: $name",
    ({ input, expected }) => {
